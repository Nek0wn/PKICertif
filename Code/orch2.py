...
def simulate_scenario_2(client):
    ...
    client.publish("vehicle/ca/request_cert", json.dumps({"vendor": "vendor_2", "symmetric_key": ca_public_key}))
    time.sleep(5)  # Attendre pour laisser le temps de réponse

    # Le vendeur envoie sa clé publique à la CA
    client.publish("vehicle/ca/vendor_public_key", json.dumps({"vendor": "vendor_2", "public_key": ca_public_key}))
    time.sleep(5)  # Attendre pour laisser le temps de réponse
